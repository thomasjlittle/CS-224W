...
from error import (
    ...
    HcountChiralError,
    MultiBondError,
    NonmatchingSymbolError,
    NonmatchingTransCisError,
    SelfLoopError,
    SmilesError,
)
...
class SMILES:
    ...
    def finalCheck(self, level, base_pos, pos, prevBond, prevAtom):
        ...
        if (
            prevBond != None
        ):  # the chain ends with an open trailing bond, append explicit hydrogen to that
        ...
        if level == 0:
            ...
            if not not self.ringDict:
                for i in self.ringDict.keys():
                    errorMsg(
                        ...
                        base_pos + pos,
                        "Error",
                        "missing ring-bond, ring ID = " + str(i) + " not closed",
                    )
                raise SmilesError
                    ...
                    for node1 in edge:
                        ...
                        for node2 in neighbors:
                            if node2 in edge:
                                ...
                                if (
                                    self.G.edges[node1, node2]["type"] != "\\"
                                    and self.G.edges[node1, node2]["type"] != "/"
                                ):
            ...
            for node in self.G.nodes():
                if (
                    self.G.nodes[node]["_type"] == "organic"
                    or self.G.nodes[node]["_type"] == "bracket_atom"
                ):
                        ...
                        if len(self.G.nodes[node]["neighList"]) != 4:
                            ...
                            for atom in self.G.nodes[node]["neighList"]:
                                if self.G.nodes[atom]["atom"] == "H":
                                    if (
                                        self.G.nodes[atom]["isotope"] == ""
                                        or self.G.nodes[atom]["isotope"] == "1"
                                    ):
                                        numH[0] += 1
    ...
    def parseError(self, tmpStr, base_pos, pos):
        if tmpStr[0] == "(":
            ...
            errorMsg(
                ...
                base_pos + pos,
                "Error",
                "in parsing branch, unbalanced parenthesis or illegal expression within branch",
            )
            raise SmilesError
        elif tmpStr[0] == ")":
    ...
    def writeLinear(self, base):
        ...
        while thisAtom != None:
            ...
            hswap, hcount = self.swapBranch(
                start,
                thisAtom,
                nextAtoms,
                lambda atom: self.G.nodes[atom]["atom"] == "H"
                and self.G.nodes[atom]["isotope"] == ""
                and self.G.nodes[atom]["charge"] == ""
                and self.G.nodes[atom]["_class"] == "",
            )
            ringswap, ringcount = self.swapBranch(
                start,
                thisAtom,
                nextAtoms,
            ...
            # write atom
            smilesStr = smilesStr + self.writeAtom(
                prevAtom, thisAtom, thisBond, rotCount, swapCount, hcount
            )
    ...
    def draw(self):
        ...
        edgeColorDict = {"": "k", "-": "k", "=": "r", "#": "purple", "/": "b", "\\": "g", ":": "k"}
        edge_colors = []
        ...
        nx.draw_networkx_labels(
            self.G, pos=position, font_weight="bold", font_color="k", labels=labelsH
        )
