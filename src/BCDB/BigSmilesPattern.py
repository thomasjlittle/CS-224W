...
class BigSmilesPattern(SmilesPattern):
    ...
    _TerminalBond = (
        Literal("[") + (_BondDesc * (0, 1)).setResultsName("BigSMILES_Bond") + Literal("]")
    ).setResultsName("BigSMILES_terminalBond")
    ...
    _StoObjLists = _StoObjList.setResultsName("repUnit") + (
        Literal(";") + _StoObjList.setResultsName("endGrp")
    ) * (0, 1)
    _StoObjLists.setParseAction(separateList)

    #### DEFINITIONS of patterns involved in BigSMILES() ####
    _BigSmilesChar = (
        SmilesPattern._smilesChar | _BigSmilesBond | _StoObj.setResultsName("BigSMILES_StoObj")
    )
