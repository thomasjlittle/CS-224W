...
class BigSMILES(SMILES):
    ...
    def addBigSmilesStoObjAtom(self, res, prevAtom, pos, prevBond):
        ...
        try:
            self.G.nodes[nodeId]["BigSMILES_StoObj"] = BigSMILES_StoObj(
                res.rawStr, pos=pos, index=self.index + [StoObjIdx], prevBond=prevBond
            )
        except:
            # errorMsg(self.rawStr,pos,'Error','Inconsistency between bonding descriptor')
                ...
                if "branch" in res.keys():
                    ...
                    if "bond" not in res.keys():
                        ...
                        if self.G.nodes[prevAtom]["BigSMILES_Bond"].getS_bond() == "u":
                            ...
                            if (
                                self.G.nodes[prevAtom]["BigSMILES_Bond"].checkConsistency(
                                    self.Bond_Dict, prevAtom
                                )
                                == -1
                            ):
                                errorMsg(
                        ...
                        if self.G.nodes[prevAtom]["BigSMILES_Bond"].getBondOrder() != self.G.nodes[
                            prevAtom
                ...
                if "branch" in res.keys():
                    ...
                    if "bond" not in res.keys():
                        ...
                        if self.G.nodes[prevAtom][
                            "BigSMILES_StoObj"
                        ].rightEnd.getBondOrder() != self.G.nodes[prevAtom][
                            "BigSMILES_StoObj"
                        ].rightEnd.getBondOrder(
                            trailing_bond
                        ):
                            errorMsg(
                                self.rawStr,
    ...
    def writeAtom(self, prevAtom, thisAtom, thisBond, rotCount, swapCount, hcount):
        ...
        #                return self.G.nodes[thisAtom]['BigSMILES_Bond'].getS_bond(isFirst=False) + smilesStr
            ...
            if len(self.G.nodes[thisAtom]["neighList"]) > 1:
                ...
                if (
                    self.G.nodes[nextAtom]["atom"] == "H"
                    and self.G.nodes[nextAtom]["_type"] == "bracket_atom"
                ):
                    smilesStr = (
                        smilesStr
                        + self.G.nodes[thisAtom]["BigSMILES_StoObj"].getBond(end="right")
                        + "[H]"
                    )
            return smilesStr
        else:
            if self.noWriteBondDesc == True and prevAtom != None:
                if self.G.nodes[prevAtom]["_type"] == "BigSMILES_Bond":
                    thisBond = ""
            return SMILES.writeAtom(
                self, prevAtom, thisAtom, thisBond, rotCount, swapCount, hcount
            )
...
# Convert BigSMILES string to graph object
def BigSmiles_to_graph(s):
    # Load BigSMILES object
    Polymer = BigSMILES(s)

    # Create graph object
    Polymer_graph = Polymer.G

    # Isolate BigSMILES objects and convert to graphs
    graph_list = []
    for obj in Polymer:
        graph = []
        for rep in obj:
            graph.append(BigSMILES(rep.writeStandard(noBondDesc=True)).G)
        graph_list.append(graph)

    # Combine adjacent BigSMILES objects into single graph
    for i in range(len(graph_list)):
        # relabel nodes to match disjoint union labeling
        graph_list[i][0] = nx.disjoint_union(graph_list[i][0], nx.Graph())

        if len(graph_list[i]) != 1:
            while len(graph_list[i]) > 1:
                connection_idx = graph_list[i][0].number_of_nodes()
                graph_list[i][0] = nx.disjoint_union(graph_list[i][0], graph_list[i][1])
                graph_list[i][0].add_edge(connection_idx - 1, connection_idx)
                graph_list[i].pop(1)
        graph_list[i] = graph_list[i][0]

    # relabel nodes in Polymer_graph to match disjoint union labeling
    Polymer_graph = nx.disjoint_union(Polymer_graph, nx.Graph())

    count = 0
    polymer_graph_total_nodes = len(Polymer_graph.nodes._nodes)
    node = 0
    while node < polymer_graph_total_nodes:
        if Polymer_graph.nodes._nodes[node]["atom"] == "BigSMILES_StoObj":
            # Define neighbors of BigSMILES monomer
            neighbors = list(Polymer_graph.neighbors(node))
            # Adjust neighbors if BigSMILES monomer is not first monomer in polymer
            if len(neighbors) > 1 and count != 0:
                neighbors[1], neighbors[0] = neighbors[0], neighbors[1]

            # Assume that each BigSMILES monomer has two or less neighbors
            if len(neighbors) > 2:
                raise Exception("BigSMILES monomer has more than two neighbors")

            # Connect neighbor[0] to first node in graph_list[0]
            number_of_BigSMILES_nodes = len(graph_list[count].nodes._nodes)
            Polymer_graph = nx.disjoint_union(Polymer_graph, graph_list[count])
            BigSMILES_node_idx = range(
                len(Polymer_graph.nodes._nodes) - number_of_BigSMILES_nodes,
                len(Polymer_graph.nodes._nodes),
            )

            Polymer_graph.add_edge(neighbors[0] - count, BigSMILES_node_idx[0])

            # Connect neighbor[1] to last node in graph_list[0]
            if len(neighbors) == 2:
                Polymer_graph.add_edge(neighbors[1] - count, BigSMILES_node_idx[-1])

            # Remove placeholder node
            Polymer_graph.remove_node(node - count)

            count += 1
        node += 1

    # Generate labels for each node
    labels = {}
    for node in Polymer_graph.nodes:
        labels[node] = Polymer_graph.nodes._nodes[node]["rawStr"]

    nx.draw(Polymer_graph, with_labels=True, labels=labels)
    return Polymer_graph
...
if __name__ == "__main__":
    ...
    plga = "CCC(C){[$][$]C\C=C(C)/C[$],[$]C\C=C(C)\C[$],[$]CC(C(C)=C)[$],[$]CC(C)(C=C)[$][$]}{[>][<][Si](C)(C)O[>][<]}[Si](C)(C)(C)"
