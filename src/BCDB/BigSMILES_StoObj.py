...
class BigSMILES_StoObj:
    ...
    def __init__(self, inStr, pos=0, index=list(), prevBond=None):
        ...
        try:
            ...
            errorMsg(
                self.rawStr,
                len(self.rawStr) - self.tmpHolder,
                "Error",
        ...
        for i in range(len(res.repUnit)):
            ...
            try:
                ...
                errorMsg(
                    ...
                    localpos,
                    "Error",
                    "error in parsing repeat unit [" + str(len(self.repUnit) + 1) + "]",
                )
                raise BigSMILES_StoObjError
            else:
                # update Bond_Dict
        ...
        for i in range(len(res.endGrp)):
            ...
            try:
                ...
                errorMsg(
                    ...
                    localpos,
                    "Error",
                    "error in parsing end group [" + str(len(self.endGrp) + 1) + "]",
                )
                raise BigSMILES_StoObjError
            else:
                # update Bond_Dict
    ...
    def parseLeftTerminal(self, peeledStr, prevBond):
        ...
        if "BigSMILES_Bondtype" not in res.keys():
            ...
            self.leftEnd = BigSMILES_Bond(
                res, S_bond=prevBond, Bond_Dict=self.Bond_Dict, item=None
            )
    ...
    def parseRightTerminal(self, peeledStr):
        ...
        if "BigSMILES_Bondtype" not in res.keys():
            ...
            try:
                self.rightEnd = BigSMILES_Bond(
                    res, S_bond="u", Bond_Dict=self.Bond_Dict, item=None
                )
            except:
